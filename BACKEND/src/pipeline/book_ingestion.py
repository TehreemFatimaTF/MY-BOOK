    ...
    async def _store_in_qdrant(self, book_id: str, chunks: list, embeddings: list):
        try:
            ...
            for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                # PURANA CODE: "id": f"{book_id}_chunk_{i}" <-- Is wajah se error aa raha tha
                
                # NAYA CODE: Har chunk ke liye ek unique UUID generate karein
                point_id = str(uuid.uuid4()) 
                
                point = {
                    "id": point_id, # Ab yeh ek valid UUID hai
                    "vector": embedding,
                    "payload": {
                        "book_id": book_id,
