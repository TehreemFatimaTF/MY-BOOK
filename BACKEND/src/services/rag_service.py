...
class RAGService:
    def __init__(self, cohere_service=None, qdrant_service=None, postgres_service=None):
        ...
        from src.services.cohere_service import cohere_service as cohere_inst
        ...
        if self.provider == "gemini":
            ...
            self.llm_service = cohere_service or cohere_inst
    ...
    async def process_query(self, user_query: UserQuery) -> GeneratedResponse:
        ...
        try:
            ...
            logger.info(f"Processing query using {self.provider}: '{user_query.query_text[:50]}...' for book {user_query.book_id}")
    ...
    async def process_query_with_validation(self, user_query: UserQuery) -> GeneratedResponse:
        ...
        try:
            # Validate query
            if len(user_query.query_text.strip()) < 1:
                raise ValueError("Query must be at least 1 characters long")
